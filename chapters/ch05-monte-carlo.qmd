...
- Monte Carlo methods are used when we do **not** have knowledge of the transition probabilities (model of the environment) and then we must learn directly from experience.
...
## 5.1 Monte Carlo Prediction {#sec-ch05-5-1}
...
## 5.2 Monte Carlo Estimation of Action Values {#sec-ch05-5-2}
...
## 5.3 Monte Carlo Control {#sec-ch05-5-3}
...
- **Policy evaluation**: MC policy eval, $Q \to q_\pi$
- **Policy improvement**: $\varepsilon$-greedy policy improvement

$$\pi \xrightarrow{\text{improvement}} \pi \to \text{greedy}(Q) \xrightarrow{\text{evaluation}} Q \to q_\pi$$
...
## 5.4 Monte Carlo Control Without Exploring Starts {#sec-ch05-5-4}
...
- To avoid having to use exploring starts, we can use two approaches, **on-policy** and **off-policy** methods. These are the only ways to ensure that all actions are selected infinitely often by the agent continuing to select them.
  ...
  - **On-policy**: Evaluate/improve the policy that is used to make the decisions
    ...
    - Uses **$\varepsilon$-greedy** policy; generally **soft** policy: $\pi(a|s) > 0\ \forall s \in S,\ a \in A(s)$ (assign nonzero probability to each possible action)
...
## 5.5 Off-Policy Prediction via Importance Sampling {#sec-ch05-5-5}
...
- Let's consider the prediction problem, in which both target & behavior policies are fixed.
  - **Goal**: Estimate $V_\pi$ or $q_\pi$, but all we have are episodes following another policy $b$, where $b \neq \pi$
    - $\pi \equiv$ target policy
    - $b \equiv$ behavior policy — both policies are considered fixed & given
...
- MC algorithm that averages returns from a batch of observed episodes following policy $b$ to estimate $V_\pi(s)$:
  - $\mathcal{J}(s) \equiv$ set of all time steps in which state $s$ is visited
  - $T(t) \equiv$ 1st time of termination following time $t$
  - $G_t \equiv$ return after $t$ up through $T(t)$
  - $\{G_t\}_{t \in \mathcal{J}(s)} \equiv$ returns that pertain to state $s$
  - $\{\rho_{t:T-1}\}_{t \in \mathcal{J}(s)} \equiv$ corresponding importance-sampling ratios
...
## 5.6 Incremental Implementation {#sec-ch05-5-6}
...
## 5.7 Off-Policy Monte Carlo Control {#sec-ch05-5-7}
...
## 5.8 Discounting-aware Importance Sampling {#sec-ch05-5-8}
...
- Let's think of discounting as determining a probability of termination, or equivalently a degree of partial termination $\gamma \in (0,1]$; $T \equiv$ time of termination of the episode.

- The **flat partial** returns:

$$\bar{G}_{t:h} \doteq R_{t+1} + R_{t+2} + \ldots + R_h, \quad 0 \leq t \leq h \leq T$$

  - where "flat" $\equiv$ absence of discounting
  - where "partial" $\equiv$ these returns do not extend all the way to termination, but instead stop at $h$, called the **horizon**
...
## 5.9 Per-decision Importance Sampling {#sec-ch05-5-9}
...
$$\mathbb{E}\left[\rho_{t:T-1}\, G_t\right] = \mathbb{E}\left[\tilde{G}_t\right] \implies \textbf{per-decision importance sampling}$$
