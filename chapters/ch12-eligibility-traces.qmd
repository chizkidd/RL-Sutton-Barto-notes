...
- Let's focus on the bootstrapping generalization of per-decision importance sampling with control variates ([Section 7.4](ch07-nstep-bootstrapping.qmd#sec-ch07-7-4)).
...
- TB($\lambda$) concept is straightforward:
  - The tree-backup updates of each length ([Section 7.5](ch07-nstep-bootstrapping.qmd#sec-ch07-7-5)) are weighted dependent on the bootstrapping parameter $\lambda$.
  - Using the recursive form of the action-based $\lambda$-return for Expected Sarsa and then expanding the bootstrapping target case after the model of tree-backup $n$-step return ([Section 7.5](ch07-nstep-bootstrapping.qmd#sec-ch07-7-5)):
