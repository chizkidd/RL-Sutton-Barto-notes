...
- Let's focus on the bootstrapping generalization of per-decision importance sampling with control variates ([Section 7.4](ch07-n-step-bootstrapping.qmd#sec-ch07-7-4)).
...
- TB($\lambda$) concept is straightforward:
  - The tree-backup updates of each length ([Section 7.5](ch07-n-step-bootstrapping.qmd#sec-ch07-7-5)) are weighted dependent on the bootstrapping parameter $\lambda$.
  - Using the recursive form of the action-based $\lambda$-return for Expected Sarsa and then expanding the bootstrapping target case after the model of tree-backup $n$-step return ([Section 7.5](ch07-n-step-bootstrapping.qmd#sec-ch07-7-5)):
...
- Extension of one-step Emphatic TD ([Sections 9.11](ch09-on-policy-prediction-approximation.qmd#sec-ch09-9-11) & [11.8](ch11-off-policy-methods-approximation.qmd#sec-ch11-11-8)) to eligibility traces.
