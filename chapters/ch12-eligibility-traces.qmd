...
## The $\lambda$-return {#sec-ch12-12-1}

- Recall in [Chapter 7](ch07-n-step-bootstrapping.qmd) we defined an $n$-step return as the sum of the first $n$ rewards plus the estimated value of the state reached in $n$ steps, each appropriately discounted:

$$G_{t:t+n} \doteq R_{t+1} + \gamma R_{t+2} + \ldots + \gamma^{n-1} R_{t+n} + \gamma^n \hat{v}(S_{t+n}, \mathbf{w}_{t+n-1}), \quad 0 \leq t \leq T - n$$
...
::: {#fig-ch12-12-1-compound}
![](../figures/ch12-12-1-the-2-and-4-step-returns.png){fig-align="center"}

Backup Diagram for Compound Update.
:::

> **Backup Diagram for Compound Update:** The compound update mixing half of a two-step return and half of a four-step return.
...
::: {#fig-ch12-12-1-tdlambda}
![](../figures/ch12-12-1-td-lambda.png){fig-align="center"}

Backup Diagram for TD($\lambda$).
:::

> **Backup Diagram for TD($\lambda$):** If $\lambda = 0$, then the overall update reduces to its first component, the **TD(0)** update, whereas if $\lambda = 1$, then the overall update reduces to its last component, the **MC** update.
...
::: {#fig-ch12-12-1-weighting}
![](../figures/ch12-12-1-td-lambda-weighting-function.png){fig-align="center"}

TD($\lambda$) Weighting.
:::

> **TD($\lambda$) Weighting:** Weighting given in the $\lambda$-return to each of the $n$-step returns.
...
::: {#fig-ch12-12-1-forward-view}
![](../figures/ch12-12-1-forward-view-td-lambda.png){fig-align="center"}

Forward View TD($\lambda$).
:::

> **Forward View TD($\lambda$):** We decide how to update each state by looking forward to future rewards and states.

---

## TD($\lambda$) {#sec-ch12-12-2}
...
\begin{aligned}
\text{where} \\
\lambda &\equiv \text{trace decay parameter} \\
\gamma &\equiv \text{discount rate}
\end{aligned}
$$
...
::: {#fig-ch12-12-2-backward-view}
![](../figures/ch12-12-2-backward-view-td-lambda.png){fig-align="center"}

Backward View of TD($\lambda$).
:::

> **Backward View of TD($\lambda$):** In the backward or mechanistic view of TD($\lambda$), each update depends on the current TD error combined with the current eligibility traces of past events.
...
## $n$-step Truncated $\lambda$-return Methods {#sec-ch12-12-3}
...
\begin{aligned}
\text{where} \\
h &\equiv \text{horizon (plays same role as time of termination } T\text{)}
\end{aligned}
$$
...
\begin{aligned}
\text{where} \\
\delta_i' &\equiv R_{t+1} + \gamma \hat{v}(S_{t+1}, \mathbf{w}_{t-1}) - \hat{v}(S_t, \mathbf{w}_{t-1})
\end{aligned}
$$

::: {#fig-ch12-12-3-ttdlambda}
![](../figures/ch12-12-3-truncated-td-lambda.png){fig-align="center"}

Backup Diagram for Truncated TD($\lambda$).
:::

> **Backup Diagram for Truncated TD($\lambda$):** The truncated $\lambda$-return gives rise to a family of $n$-step $\lambda$-return algorithms called **TTD($\lambda$)**.

---

## Redoing Updates: Online $\lambda$-return Algorithm {#sec-ch12-12-4}
...
## True Online TD($\lambda$) {#sec-ch12-12-5}

- The original ideal online $\lambda$-return algorithm shown in [Section 12.4](#sec-ch12-12-4) is very complex so we use online TD($\lambda$) to approximate it.
...
## Dutch Traces in Monte Carlo Learning {#sec-ch12-12-6}
...
::: {#fig-ch12-12-7-sarsa-lambda}
![](../figures/ch12-12-7-sarsa-lambda.png){fig-align="center"}

Backup Diagram for Sarsa($\lambda$).
:::

> **Backup Diagram for Sarsa($\lambda$):** The first update looks ahead one full step, to the next state-action pair, the second looks ahead two steps, to the second state-action pair, and so on. A final update is based on the complete return.
...
- There exists an action-value version of our ideal TD method, the online $\lambda$-return algorithm and its efficient implementation as true online TD($\lambda$):
  - [Section 12.4](#sec-ch12-12-4): Everything there holds here except for using the action-value form of the $n$-step return, $G_{t:t+n}$.
  - [Sections 12.5](#sec-ch12-12-5) & [12.6](#sec-ch12-12-6): Everything holds here except for using state-action feature vectors $\mathbf{x}_t = \mathbf{x}(S_t, A_t)$ instead of state feature vectors $\mathbf{x}_t = \mathbf{x}(S_t)$.
...
## Variable $\lambda$ and $\gamma$ {#sec-ch12-12-8}
...
\begin{aligned}
\text{where} \\
\prod_{k=t}^{\infty} \gamma_k &= 0 \text{ with probability 1 for all } t, \text{ to assure the sums are finite}
\end{aligned}
$$

- This general return $G_t$ definition enables episodic settings to become a single stream of experience, without special terminal states, start distributions, or termination times.
...
\begin{aligned}
\text{where} \\
\bar{V}_t(s) &\doteq \sum_a \Pi(a \vert s)\, \hat{q}(s, a, \mathbf{w}_t)
\end{aligned}
$$
...
## Off-Policy Traces with Control Variates {#sec-ch12-12-9}

- To generalize to off-policy, we need to incorporate importance sampling using eligibility traces.
- Let's focus on the bootstrapping generalization of per-decision importance sampling with control variates ([Section 7.4](https://chizkidd.github.io/RL-Sutton-Barto-notes/chapters/ch07-nstep-bootstrapping.html#sec-ch07-7-4)).
- The new state-based $\lambda$-return in [Section 12.8](#sec-ch12-12-8) generalizes, after the off-policy, control variate, $n$-step return (ending at horizon $h$) model to:
...
- This eligibility trace combined with the usual semi-gradient TD($\lambda$) parameter-update rule ([Section 12.2](#sec-ch12-12-2)) forms a **general TD($\lambda$)** algorithm that can be applied to either on-policy or off-policy data:
...
\begin{aligned}
\text{where} \\
\bar{V}_t(S_{t+1}) &= \sum_a \Pi(a \vert S_{t+1})\, \hat{q}(S_{t+1}, a, \mathbf{w}_t)
\end{aligned}
$$
...
- This ET combined with the action-based, expected TD error $\delta_t^a$ and the usual semi-gradient TD($\lambda$) parameter-update rule ([Section 12.2](#sec-ch12-12-2)) forms an elegant, efficient **Expected Sarsa($\lambda$)** algorithm that can be applied to either on-policy or off-policy data:
...
- Algorithmic strategies for handling the 2nd part of the off-policy learning challenge with eligibility traces are summarized in [Section 12.11](#sec-ch12-12-11).

---

## Watkins's Q($\lambda$) to Tree-Backup($\lambda$) {#sec-ch12-12-10}
...
::: {#fig-ch12-12-10-watkins}
![](../figures/ch12-12-10-watkins-q-lambda.png){fig-align="center"}

Backup Diagram for Watkins's Q($\lambda$).
:::

> **Backup Diagram for Watkins's Q($\lambda$):** The series of component updates ends either with the end of the episode or with the first nongreedy action, whichever comes first.
...
- TB($\lambda$) concept is straightforward:
  - The tree-backup updates of each length ([Section 7.5](https://chizkidd.github.io/RL-Sutton-Barto-notes/chapters/ch07-nstep-bootstrapping.html#sec-ch07-7-5)) are weighted dependent on the bootstrapping parameter $\lambda$.
  - Using the recursive form of the action-based $\lambda$-return for Expected Sarsa and then expanding the bootstrapping target case after the model of tree-backup $n$-step return ([Section 7.5](https://chizkidd.github.io/RL-Sutton-Barto-notes/chapters/ch07-nstep-bootstrapping.html#sec-ch07-7-5)):
...
::: {#fig-ch12-12-10-tree-backup}
![](../figures/ch12-12-10-tree-backup-q-lambda.png){fig-align="center"}

Backup Diagram for Tree-Backup($\lambda$).
:::

> **Backup Diagram for Tree-Backup($\lambda$):** The tree-backup updates of each length are weighted in the usual way dependent on the bootstrapping parameter $\lambda$.
...
## Stable Off-Policy Methods with Traces {#sec-ch12-12-11}
...
- GQ($\lambda$) aims to learn $\mathbf{w}_t$ such that $\hat{q}(s, a, \mathbf{w}_t) \doteq \mathbf{w}_t^T \mathbf{x}(s,a) \approx q_\pi(s,a)$ from off-policy data.
...
- Extension of one-step Emphatic TD ([Sections 9.11](https://chizkidd.github.io/RL-Sutton-Barto-notes/chapters/ch09-on-policy-prediction-approximation.html#sec-ch09-9-11) & [11.8](https://chizkidd.github.io/RL-Sutton-Barto-notes/chapters/ch11-off-policy-methods-approximation.html#sec-ch11-11-8)) to eligibility traces.
...
## Implementation Issues {#sec-ch12-12-12}
