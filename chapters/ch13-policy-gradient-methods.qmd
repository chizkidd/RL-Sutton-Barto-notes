...
- Almost all the algorithms/methods covered so far have been **action-value methods** (except gradient-bandit algorithms, [Section 2.8](ch02-multi-armed-bandits.qmd#sec-ch02-2-8)).
...
## Policy Approximation & Its Advantages {#sec-ch13-13-1}
...
## The Policy Gradient Theorem {#sec-ch13-13-2}
...
## REINFORCE: Monte Carlo Policy Gradient {#sec-ch13-13-3}
...
### Pseudocode: REINFORCE (Episodic) for estimating $\pi_*$
...
## REINFORCE with Baseline {#sec-ch13-13-4}
...
### Pseudocode: REINFORCE with Baseline (Episodic), for estimating $\pi_{\boldsymbol{\theta}} \approx \pi_*$
...
- Choosing $\alpha^{\mathbf{w}}$ is relatively easy; in the linear case a good rule of thumb is (see [Section 9.6](ch09-on-policy-prediction-approximation.qmd#sec-ch09-9-6)):
...
## Actor-Critic Methods {#sec-ch13-13-5}
...
### One-Step Actor-Critic Methods {#sec-ch13-13-5-1}
...
### Pseudocode: One-Step Actor-Critic (Episodic), for estimating $\pi_{\boldsymbol{\theta}} \approx \pi_*$
...
### Actor-Critic with Eligibility Traces {#sec-ch13-13-5-2}
...
### Pseudocode: Actor-Critic with ET (Episodic), for estimating $\pi_{\boldsymbol{\theta}} \approx \pi_*$
...
## Policy Gradient for Continuing Problems {#sec-ch13-13-6}

- From [Section 10.3](ch10-on-policy-control-approximation.qmd#sec-ch10-10-3) on continuing problems, lack of episode boundaries requires a new performance measure definition in terms of the **average rate of reward per time step**:
...
Leave the notation implicit in all cases that $\pi = f(\boldsymbol{\theta})$ and that the gradients $\nabla[\ldots]$ are w.r.t. $\boldsymbol{\theta}$. In the continuing case $J(\boldsymbol{\theta}) = r(\pi)$, and $v_\pi$ & $q_\pi$ denote values w.r.t. the **differential return**.
...
### Pseudocode: Actor-Critic with ET (Continuing), for estimating $\pi_{\boldsymbol{\theta}} \approx \pi_*$
...
## Policy Parametrization for Continuous Actions {#sec-ch13-13-7}
...
::: {#fig-ch13-13-7-pdf}
![](../figures/ch13-13-7-prob-density-function.png){fig-align="center"}

Probability Density Function (PDF).
:::

> **Probability Density Function (PDF):** The probability density functions for several different means and standard deviations are shown above.
