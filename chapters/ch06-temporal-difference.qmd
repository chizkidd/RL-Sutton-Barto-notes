...
## 6.1 TD Prediction {#sec-ch06-6-1}
...
where $G_t \equiv$ actual return following time $t$, and $\alpha \equiv$ constant step-size parameter.
...
The TD error at each time is the error in the estimate made at that time. If the array $V$ does not change during the episode (like in MC methods), then:
...
## 6.2 Advantages of TD Prediction Methods {#sec-ch06-6-2}
...
- TD methods can learn **before** knowing the final outcome & **without** the final outcome, unlike MC:
  - (a) TD can learn online, incrementally, while MC must wait until the episode ends when the return is known.
  - (b) TD can learn from incomplete sequences/episodes while MC only learns from complete sequences. TD works in continuing (non-terminating) environments while MC only works for episodic/terminating environments.
...
## 6.3 Optimality of TD(0) {#sec-ch06-6-3}
...
- In general, batch MC methods always find the estimates that **minimize mean square error** on the training set, whereas batch TD(0) always finds the estimates that would be exactly correct for the **maximum-likelihood estimate** model of the Markov process.
...
## 6.4 Sarsa: On-policy TD Control {#sec-ch06-6-4}
...
- For on-policy control, we learn an action-value function $q_\pi(s,a)$ for our current policy $\pi$, for all states $s$ and actions $a$.

Episode: $\ldots \to (S_t) \xrightarrow{A_t} (S_{t+1}) \xrightarrow{A_{t+1}} (S_{t+2}) \xrightarrow{A_{t+2}} \ldots$

$$Q(S_t, A_t) \leftarrow Q(S_t, A_t) + \alpha\Big[\underbrace{R_{t+1} + \gamma Q(S_{t+1}, A_{t+1})}_{\text{TD target}} - Q(S_t, A_t)\Big]$$

- This update rule uses a quintuple of events $(S_t, A_t, R_{t+1}, S_{t+1}, A_{t+1})$. Hence, the name **SARSA**.
...
**Goal**: Use TD in the control loop — apply TD to $Q(s,a)$, use $\varepsilon$-greedy policy improvement, update with every time-step.
...
## 6.5 Q-learning: Off-Policy TD Control {#sec-ch06-6-5}
...
- Q-learning is off-policy so it updates the value function greedily directly while updating/exploring the other actions with the $\varepsilon$-greedy policy.
  ...
  - Under the GLIE assumption (all state-action pairs continue to be updated) and following the stochastic approximation theory variant on the sequence of $\alpha$, $Q$ has been shown to converge with probability 1 to $q_*$:
...
- We improve both policies (target & behavior):
  - Improve target policy $\pi$ greedily w.r.t. $Q(s,a)$: $\pi(S_{t+1}) = \arg\max_{a'} Q(S_{t+1}, a')$
...
## 6.6 Expected Sarsa {#sec-ch06-6-6}

- Instead of updating $Q(S,A)$ with the value maximising action at $S_{t+1}$, $\max_a Q(S_{t+1}, a)$ as used in Q-learning, or the greedy $\varepsilon$-greedy exploratory action as in SARSA, we can use the **expected value** of $Q$ at $S_{t+1}$, taking into account how likely each action is under the current policy. The update rule is:
...
## 6.7 Maximization Bias & Double Learning {#sec-ch06-6-7}
...
- If the true values of state-action pairs $q(s,a)$ of a single state $s$ with multiple actions $a$ are all zero, i.e. $q(s,a) = 0\ \forall s, a$, but whose estimated values $Q(s,A)$ are uncertain and thus distributed some above & some below zero, then the maximum of the true values is zero, but the maximum of the estimates is positive — a positive bias.
...
## 6.8 Games, Afterstates & Other Special Cases {#sec-ch06-6-8}
...
- Afterstate methods are aptly described in terms of GPI, with a policy and afterstate value function interacting in essentially the same way.
  - Still have to decide on/off policy for exploration / persistent exploration need management.

---

## 6.9 Summary {#sec-ch06-6-9}
