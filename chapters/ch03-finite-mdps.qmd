...
In a **finite** MDP, the sets of states $S$, actions $A$, and rewards $R$ are all finite. The random variables $S_t$ and $R_t$ have well-defined discrete probability distributions dependent only on the preceding state and action, this is the **Markov property**. The transition dynamics are fully characterized by the function $p$, which gives the probability of transitioning to state $s' \in S$ and receiving reward $r \in R$, given state $s$ and action $a$.


$$
\begin{align*}
p(s', r \vert s, a) &\doteq \mathbb{P}\{S_t = s',\, R_t = r \mid S_{t-1} = s,\, A_{t-1} = a\} \\
& \quad \forall\, s', s \in S,\, r \in \mathbb{R},\, a \in A(s) \\[6pt]
p &: S \times R \times S \times A \to [0,1] \\[6pt]
...
- From the 4-argument dynamics function, $p$, we can compute anthing we want to know about the environment. Let's go through the variables of interest below.
...
- From $G_t = \sum_{k=0}^{\infty} \gamma^k R_{t+k+1}$, the expected return is finite if $R \neq 0 \text{ and constant, if } \gamma < 1$: 
  - For example, if $R = \text{a constant } + 1$ at each timestep, then the discounted return is:
...
- "$\vert$" in $\pi(a|s) \Rightarrow$ defines a probability distribution over $a \in A(s)$ for each $s \in S$.
...
- The **return** is the (expected) function of future rewards the agent seeks to maximize (undiscounted for episodic tasks, discounted for continuing tasks).
