...
## 7.1 $n$-step TD Prediction — $\text{TD}(\lambda)$ {#sec-ch07-7-1}
...
## 7.2 $n$-step Sarsa — $\text{Sarsa}(\lambda)$ {#sec-ch07-7-2}
...
$$Q_{t+n}(S_t, A_t) \leftarrow Q_{t+n-1}(S_t, A_t) + \alpha\left[G_t^{(n)} - Q_{t+n-1}(S_t, A_t)\right]$$
...
## 7.3 $n$-step Off-policy Learning {#sec-ch07-7-3}
...
$$Q(S_t, A_t) \leftarrow Q(S_t, A_t) + \alpha\, \rho_{t+1:t+n-1}\left[G_t^{(n)} - Q(S_t, A_t)\right]$$
...
## 7.4 Per-decision Methods with Control Variates {#sec-ch07-7-4}
...
$$G_{t:h} \doteq \rho_t\left(R_{t+1} + \gamma G_{t+1:h}\right) + (1 - \rho_t) V_{h-1}(S_t), \quad t < h < T,\ G_{h:h} \doteq V_{h-1}(S_h)$$
...
## 7.5 Off-policy Learning Without Importance Sampling: The $n$-step Tree Backup Algorithm {#sec-ch07-7-5}
...
$$\text{for } t < T-1,\ n \geq 2$$

- The $n$-step tree-backup return is:

$$G_t^{(n)} \doteq R_{t+1} + \gamma \sum_{a \neq A_{t+1}} \pi(a|S_{t+1})\, Q_{t+n-1}(S_{t+1}, a) + \gamma\pi(A_{t+1}|S_{t+2})\, G_{t+1:t+n}$$
...
## 7.6 A Unifying Algorithm: $n$-step $Q(\sigma)$ {#sec-ch07-7-6}
...
- **Unification Algorithm** $\Rightarrow$ One Idea: Decide on a step-by-step basis whether we want to sample, as in Sarsa, or consider the expectation over all actions instead, as in tree-backup update.
...
- After which it is exactly like the $n$-step return for Sarsa with control variates except with the action probability $\pi(A_{t+1}|S_{t+1})$ substituted for the importance sampling ratio $\rho_{t+1}$.

For $Q(\sigma)$, we slide linearly between these 2 cases: $\forall\, t < h \leq T$
...
- The 2 4-step backup diagrams below together summarize most of the methods introduced: $n$-step TD with importance sampling, and $n$-step $Q(\sigma)$ which generalizes Expected Sarsa & Q-learning.
...
- $n$-step methods, while being quite complex, are conceptually clear. Hence the development of 2 $n$-step off-policy learning approaches based on:
