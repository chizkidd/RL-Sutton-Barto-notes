...
::: {.callout-note title="Multi-Armed Bandit"}
A multi-armed bandit is a tuple $\langle A, R \rangle$ 
where $A$ is a known set of $m$ actions or **"arms"** and 
$R^a(r)$ is an unknown probability distribution over rewards.
:::
---
...
::: {.callout-note title="Optimistic Initialization"}
_**Assume the best action-value initially, until proven otherwise.**_
Basically **optimistic initialization** encourages systematic exploration 
early on but can still lock onto suboptimal action.
:::
...
::: {.callout-note title="Optimism in the Face of Uncertainty"}
_**The more uncertain we are about an action-value, the more important it is to explore 
that action, because it could turn out to be the best action.**_
...
::: {.callout-note title="Contextual Bandit"}
A contextual bandit is a tuple $\langle A, S, R \rangle$ 
where $A$ is a known set of $m$ actions or **"arms"**, 
$S$ is an unknown probability distribution over states or **"contexts"**
and $R^a(r)$ is an unknown probability distribution over rewards.
:::
